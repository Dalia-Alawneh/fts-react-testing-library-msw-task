...
describe("SignUp Component", () => {
  describe("Validation", () => {
    it("should display validation errors for invalid email", async () => {
      ...
      fireEvent.change(emailField, { target: { value: 'dalia-' } });
      fireEvent.blur(emailField)
    ...
    it("should display validation errors for short password", async () => {
      ...
      expect(await screen.findByText(/Password should be of minimum 8 characters length/i)).toBeInTheDocument();
    });
  ...
  describe("Form Interaction", () => {
    ...
    it("should update form fields on user input", async () => {
      render(<SignUp />);
      fillSignUpForm("test", "test@example.com", "12345678");
      expect((screen.getByLabelText(/user name/i) as HTMLInputElement).value).toBe("test")
      expect((screen.getByLabelText(/email address/i) as HTMLInputElement).value).toBe("test@example.com")
      expect((screen.getByLabelText(/password/i) as HTMLInputElement).value).toBe("12345678")
    });
